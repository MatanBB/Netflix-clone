...
function login() {
  ...
  const onSubmit: SubmitHandler<Inputs|FieldValues> = async ({email, password}) => {
    if (login) {
  ...
  return (
    ...
    md:justify-center md:bg-transparent">
      ...
      <div className="absolute left-2 top-1 h-20 w-44 cursor-pointer md:left-8 md:top-4">
        <Image src="https://rb.gy/ek4j9f" fill style={{ objectFit: "contain" }} alt="logo" />
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="relative mt-24 space-y-8 rounded bg-black/75 py-10 px-6
       md:mt-0 md:max-w-md md:p-x-14">
