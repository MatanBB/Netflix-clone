...
function Modal() {
  ...
  useEffect(() => {
    ...
    async function fetchMovie() {
      ...
      if (data?.videos) {
        const idx = data.videos.results.findIndex((element: Element) => element.type === 'Trailer')
        setTrailer(data.videos?.results[idx].key)
      }
  ...
  return (
    <MuiModal open={showModal} onClose={handleClose}
      className='fixed !top-7 left-0 right-0 mx-auto z-50 w-full max-w-5xl overflow-hidden
    overflow-y-scroll rounded-md scrollbar-hide'>
      <>
        <button onClick={handleClose} className='modalButton absolute right-5 top-50 !z-40 h-9 w-9 
        ...
        <div className='flex space-x-16 rounded-b-md bg-[#181818] px-10 py-8'>
          <div className='space-y-6 text-lg'>
            <div className='flex items-center space-x-2 text-sm'>
              <p className='font-semibold text-green-400'>{(String(movie!.vote_average * 10).slice(0,4))}% Match</p>
            ...
            <div className='flex flex-col gap-x-10 gap-y-4 font-light md:flex-row'>
              ...
              <div className='flex flex-col space-y-3 text-sm'>
                <div>
                  ...
                  <span className='text-[gray]'>Genres:
